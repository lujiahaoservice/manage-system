      ...
      <div>
        ...
        <div class="table_tabs">
          <div class="tabs_position">
            <div :class="['tab', 'tab1', tab_active === 1 ? 'active_tab' : '']" @click="changeTab(1)">抽成收入</div>
            <div :class="['tab', 'tab1', tab_active === 2 ? 'active_tab' : '']" @click="changeTab(2)">会员费收入</div>
          </div>
        </div>
        <!-- 表格列表 -->
...
<script>
import { agentIncome, agentForm } from '@/api/finance'
import { agentExport, downloadFile } from '@/api/export'
      ...
      agentExport(data).then(res => {
        if (res.data.data.data) {
          ...
          downloadFile(data1).then(res => {
            console.log(res)
            download(res.request.responseURL, '合伙人收入明细')
          })
        } else {
