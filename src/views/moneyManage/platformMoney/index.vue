    ...
    <div class="business_content">
      <div>
        <div class="content_tit">
         ...
         <div class="plat_top">
          <div class="plat_money_all">
            <div class="plat_money_all_number">
              {{(balance/100).toFixed(2)}}
              <span>元</span>
            </div>
            <div class="plat_money_all_text">
              平台余额
              <!-- <el-button type="primary" size="mini" style="margin-left:120px;">
                提现
              </el-button> -->
            </div>
          </div>
        ...
        <div class="table_tabs">
          <div class="tabs_position">
            <div :class="['tab', 'tab1', tab_active === 1 ? 'active_tab' : '']" @click="changeTab(1)">平台抽成收入</div>
            <div :class="['tab', 'tab1', tab_active === 2 ? 'active_tab' : '']" @click="changeTab(2)">平台会员费收入</div>
            <div :class="['tab', 'tab1', tab_active === 3 ? 'active_tab' : '']" @click="changeTab(3)">合伙人抽成收入</div>
            <div :class="['tab', 'tab1', tab_active === 4 ? 'active_tab' : '']" @click="changeTab(4)">合伙人会员费收入</div>
          </div>
        </div>
        <!-- 表格列表 -->
...
<script>
import { platformIncome, platformForm, platformBalance } from '@/api/finance'
import { platformExport, downloadFile } from '@/api/export'
      ...
      platformExport(data).then(res => {
        if (res.data.data.data) {
          ...
          downloadFile(data1).then(res => {
            download(res.request.responseURL, '平台收入明细')
          })
        } else {
