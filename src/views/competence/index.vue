    ...
    <div class="employeesId" v-if="!exmin">
      ...
      <div class="employeesIdRight">
        ...
        <div class="employbtn" v-if="!organization">
          ...
          <el-button type="primary" @click="dimissionrol">删除成员</el-button>
          <el-button type="primary" @click="authority">功能权限</el-button>
          <el-input
            style="width:250px;margin-left:50px;margin-right:20px;"
            size="mini"
            placeholder="请输入用户名/手机号/账号查询"
            v-model="search"
          ></el-input>
        ...
        <div class="employtab">
          ...
          >
            ...
            <el-table-column
              ...
              label="所属角色"
              align="center"
              v-if="this.search || (this.search === 0)"
            ></el-table-column>
      ...
      <div class="pub_top1">
        <!-- 添加角色 -->
        <el-dialog title="添加角色" :close-on-click-modal="false" :visible.sync="adrol" width="30%">
          <el-form :model="adrols" :rules="adrolrus" ref="adrols">
            <div class="content">
              <el-form-item label="名称" prop="appellative">
                <el-input v-model="adrols.appellative" placeholder="请输入角色名称"></el-input>
              </el-form-item>
            </div>
            <div class="content">
        ...
        <!-- 功能权限 -->
        <el-dialog title="功能" :visible.sync="Authority" :close-on-click-modal="false">
          <div class="content">
            <el-form>
        ...
        >
          ...
          <div class="input_set">
            <el-form-item label="登录账号" prop="user" v-if="!showuser">
              <div class="intt">
                <el-input class="inptseclet" v-model="ruleForm.user" placeholder="建议姓名拼音小写"></el-input>
              </div>
            </el-form-item>
          </div>
          <div class="input_set">
...
import { getMenutree, getRolePeople } from "@/api/competence";
...
export default {
  ...
  methods: {
    ...
    powerconfirm() {
      ...
      const data = {
        roleId: this.rolid,
        menuIds: this.$refs.treepowers
          .getCheckedKeys()
          .concat(this.$refs.treepowers.getHalfCheckedKeys())
      };
    ...
    dimissionrol() {
      if (
        ...
        this.$confirm(
          ...
          .then(() => {
            const data = {
              ids: this.multipleSelection[0].userId
            };
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          const id = Number(this.ruleForm.role_based.join().split(",")[0]);
          var rolename = "";
          this.role_based.map(item => {
            if (item.roleId === id) {
    ...
    addMembers() {
      ...
      getRolelist().then(res => {
        this.role_based = res.data.data;
        console.log('role',this.role_based)
      });
    ...
    searchs() {
      ...
      getRolePeople(data).then(res => {
        console.log("后台用户", res);
        this.tableData = res.data.data.records;
        this.total = res.data.data.total;
      });
    },
        ...
        <span class="custom-tree-node">
          ...
          <span>
            <el-button
              ...
              style="margin-left:4px;"
              icon="el-icon-edit"
              on-click={() => this.rolnodeEdit(data)}
            ></el-button>
            <el-button
              size="mini"
              type="text"
              style="margin-left:4px;"
              icon="el-icon-delete"
              on-click={() => this.rolremove(node, data)}
            ></el-button>
          </span>
        </span>
      );
