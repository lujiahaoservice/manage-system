    ...
    <div class="business_content">
      ...
      <div class="table_tabs">
        <div class="tabs_position">
          <div
            :class="['tab', 'tab1', tab_active === '0' ? 'active_tab' : '']"
            @click="changeTab('0')"
          >待审核</div>
          <div
            :class="['tab', 'tab1', tab_active === '1' ? 'active_tab' : '']"
            @click="changeTab('1')"
          >审核通过</div>
          <div
            :class="['tab', 'tab1', tab_active === '2' ? 'active_tab' : '']"
            @click="changeTab('2')"
          >审核未通过</div>
        </div>
      </div>
      ...
      <div class="content_form">
        ...
        >
          ...
          <el-table-column prop="created" align="center" width="180" label="创建时间"></el-table-column>
          ...
          <el-table-column prop="alipayAccount" align="center" label="提现账户">
            <template slot-scope="scope">
          ...
          <el-table-column align="center" label="提现金额">
            <template slot-scope="scope">
              ...
              >{{(scope.row.amount/100).toFixed(2)}}</div>
            </template>
          </el-table-column>
          <el-table-column
            v-if="tab_active === '0'"
            ...
            <template slot-scope="scope">
              ...
              >
                <el-button
                  ...
                  @click.native.prevent="pass(scope.$index, tableData, 1)"
                  type="primary"
                  size="small"
                >通过</el-button>
                <el-button
                  v-if="scope.row.type === 0"
                  @click.native.prevent="pass(scope.$index, tableData, 2)"
                  type="danger"
                  size="small"
                >不通过</el-button>
              </div>
...
<script>
import { withDrawalList, withDrawalUpdata } from "@/api/exmain";
  ...
  methods: {
    ...
    changeTab(type) {
      ...
      if (type === "0") {
        ...
        this.userWithdrawal_time = [fiveAgoToday, today];
      }
      this.initData();
    },
    initData(startTime, endTime) {
      const data = {
        ...
        type: this.type,
        startTime: startTime || this.userWithdrawal_time[0],
        endTime: endTime || this.userWithdrawal_time[1]
      };
      withDrawalList(data).then(res => {
        if (res.status === 200) {
          this.tableData = res.data.data.records;
          this.total = res.data.data.total;
          console.log("fanhuideshuju", this.tableData);
        }
      });
    },
    changePage(e) {
